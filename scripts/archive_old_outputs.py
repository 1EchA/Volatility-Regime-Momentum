...
def get_target_archive_dir(file_path: Path) -> Path:
    ...
    if "predictions" in filename:
        ...
        pattern in filename for pattern in ["grid", "cost_sensitivity", "turnover"]
    ):
...
def should_archive_file(
    file_path: Path, pattern_config: dict, cutoff_date: datetime
) -> bool:
    """判断文件是否应该被归档"""
...
def archive_files_by_pattern(
    pattern: str, config: dict, cutoff_date: datetime, dry_run: bool = True
) -> tuple:
    """按模式归档文件"""
    ...
    logger.info(
        f"Pattern {pattern}: {len(files)} total files, keeping {kept_count} latest"
    )
        ...
        if should_archive_file(file_path, config, cutoff_date):
            ...
            if dry_run:
                ...
                try:
                    ...
                    logger.info(
                        f"Moved: {file_path.name} -> {target_dir.name}/{target_path.name}"
                    )
                    archived_count += 1
