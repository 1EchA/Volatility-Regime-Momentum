...
def download_prices(codes: List[str], start: str, end: str):
    ...
    dc.download_stock_batch(
        codes, start_date=start, end_date=end, max_workers=5, batch_size=20
    )


def run_pipeline(
    recompute_factors: bool,
    recompute_regime: bool,
    top_n: int,
    bottom_n: int,
    cost_bps: float,
    start_oos: str,
...
def main():
    p = argparse.ArgumentParser(
        description="Restore data directory with fresh artifacts"
    )
    ...
    p.add_argument(
        "--limit", type=int, default=500, help="limit number of stocks to download"
    )
