...
def scatter_ir_vs_turnover(grid_path: Path, output: Path):
    ...
    if "avg_turnover" not in df.columns:
        raise ValueError(
            "Grid file lacks avg_turnover — use turnover_strategy_grid_*.csv"
        )
...
def heatmap_by_strategy(grid_path: Path, strategy: str, metric: str, output: Path):
    ...
    pivot = df.pivot_table(
        index="top_n", columns="cost_bps", values=metric, aggfunc="max"
    )
...
def main():
    ...
    p1.add_argument(
        "--strategy", default="B_hysteresis", help="Strategy filter for heatmap"
    )
    ...
    p2.add_argument(
        "--timeseries", required=True, help="pipeline_execution_*_timeseries.csv"
    )
    ...
    if args.cmd == "grid":
        ...
        heatmap_by_strategy(
            ...
            args.strategy,
            args.metric,
            out_dir / f"turnover_heatmap_{args.strategy}_{args.metric}_{ts}.png",
        )
        ...
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        execution_profile(
            ts_path, args.top_n, args.bottom_n, out_dir / f"exec_profile_{ts}"
        )
