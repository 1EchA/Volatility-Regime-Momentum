...
def main():
    ...
    p.add_argument(
        "--predictions", type=str, default=None, help="Glob(s) for predictions CSV"
    )
    p.add_argument(
        "--execution-metrics",
        type=str,
        default=None,
        help="Glob(s) for execution *_metrics.json",
    )
    p.add_argument(
        "--execution-timeseries",
        type=str,
        default=None,
        help="Glob(s) for execution *_timeseries.csv",
    )
    p.add_argument(
        "--turnover-grid",
        type=str,
        default=None,
        help="Glob(s) for turnover_strategy_grid_*.csv",
    )
    p.add_argument(
        "--robustness",
        type=str,
        default=None,
        help="Glob(s) for robustness_summary_*.csv",
    )
    p.add_argument(
        "--visuals",
        type=str,
        default="data/turnover_*.png,data/exec_profile_*_profile.png",
        help="Additional visuals to include (comma-separated globs)",
    )
    args = p.parse_args()
    ...
    with open(out_dir / "README.md", "w", encoding="utf-8") as f:
        ...
        if summary["highlights"]:
            ...
            for h in summary["highlights"]:
                f.write(
                    f"- {h['file']}: 策略={h.get('strategy')}, 参数={h.get('param')}, 年化={h.get('ls_ann'):.2%} IR={h.get('ls_ir'):.3f}, 换手={h.get('avg_turnover'):.2%}, 回撤={h.get('max_drawdown'):.2%}\n"
                )
