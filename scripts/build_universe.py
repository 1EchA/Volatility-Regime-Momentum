...
def load_local_candidates() -> pd.DataFrame:
    ...
    for p in DATA.glob("*.csv"):
        ...
        try:
            ...
            amt = pd.to_numeric(
                df.get("amount", pd.Series([0] * len(df))), errors="coerce"
            )
...
def load_spot_info() -> pd.DataFrame:
    ...
    try:
        ...
        # 兼容列名
        code_col = next(
            (c for c in ["代码", "股票代码", "证券代码", "code"] if c in spot.columns), None
        )
        name_col = next(
            (c for c in ["名称", "股票名称", "证券名称", "name"] if c in spot.columns), None
        )
        ffmv_col = next(
            (
                c
                for c in ["流通市值", "流通市值-亿", "流通市值(亿)", "流通市值_亿元", "流通值"]
                if c in spot.columns
            ),
            None,
        )
        mv_col = next(
            (c for c in ["总市值", "总市值-亿", "总市值(亿)", "总市值_亿元"] if c in spot.columns), None
        )
        out = pd.DataFrame()
...
def load_existing_universe() -> pd.DataFrame:
    ...
    try:
        ...
        ind_col = (
            ...
            else ("行业" if "行业" in df.columns else None)
        )
        out = pd.DataFrame()
        out["code"] = df["code"]
        if name_col:
...
def main():
    ...
    ap.add_argument(
        "--download-missing",
        action="store_true",
        help="use DataCollector to fetch missing price CSVs",
    )
    args = ap.parse_args()

    # 1) candidates from local CSVs
    local_df = load_local_candidates()
    # argparse converts --download-missing -> download_missing
    if local_df.empty and args.download_missing:
        try:
            ...
            if codes:
                dc.download_stock_batch(
                    codes[: args.target_size * 2], max_workers=5, batch_size=20
                )
        except Exception:
            pass
        local_df = load_local_candidates()
    ...
    # 排名：流通市值 → 中位成交额 → 交易天数
    uni = uni.sort_values(
        ["流通市值_亿元", "median_amount", "days"], ascending=[False, False, False]
    )
    ...
    if len(uni) < args.target_size and not existing.empty:
        ...
        uni = (
            pd.concat([uni, add], ignore_index=True)
