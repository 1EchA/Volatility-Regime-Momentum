...
def _load_dataframe(
    ...
    if data_file:
        ...
        if not latest:
            raise FileNotFoundError(
                "Unable to locate volatility regime dataset. Specify --data-file explicitly."
            )
        path = Path(latest)
    if not path.exists():
    ...
    if industry_neutral:
        if "industry" not in df.columns:
            raise ValueError(
                "Dataset lacks industry column but --industry-neutral was enabled"
            )
        ...
        def adjust(group: pd.DataFrame) -> pd.DataFrame:
            ...
            if "forward_return_1d" in local.columns:
                ...
                local["forward_return_1d"] = (
                    local["forward_return_1d"] - shrink * mean_ret
                )
            return local
...
def run_cost_grid(
    ...
    combinations = []
    for win, alpha, top_n, cost in product(
        train_windows, alphas, top_ns, cost_bps_list
    ):
    ...
    for win, alpha, top_n, bottom_n, cost in combinations:
        ...
        if preds.empty:
            ...
            metrics = evaluate_predictions(
                preds, top_n=top_n, bottom_n=bottom_n, cost_bps=cost
            )
        record = {
            "train_window": win,
            "alpha": alpha,
...
def main():
    parser = argparse.ArgumentParser(
        description="Cost sensitivity grid search for regime predictive model"
    )
    parser.add_argument(
        "--data-file",
        type=str,
    ...
    parser.add_argument(
        "--cost-bps",
        type=str,
        default="0.0003,0.0005",
        help="Comma-separated costs in decimal form (e.g. 0.0003=3bp)",
    )
    parser.add_argument(
        "--bottom-n-ratio",
        type=float,
        default=1.0,
        help="Bottom bucket as proportion of top_n (default 1.0)",
    )
    ...
    parser.add_argument(
        "--no-regime",
        action="store_true",
        help="Force pooled model instead of regime-specific mapping",
    )
