    ...
    if data_file:
        ...
        if latest is None:
            raise FileNotFoundError(
                "No volatility_regime_data_*.csv found under data/."
            )
...
def _industry_stability_stats(df: pd.DataFrame) -> dict[str, float]:
    ...
    dominant = df.groupby("stock_code")["industry"].agg(
...
def run_industry_validation(
    ...
    neutral_industries: set[str] | None = None,
) -> tuple[dict, Path]:
    ...
    if "industry" in df.columns:
        neutral_df = _neutralize_by_industry(
            df, used_cols, shrink=neutral_shrink, industries=neutral_industries
        )
        scenario_results.append(
            _evaluate(
                neutral_df,
    ...
    if (
        ...
        and scenario_results[0].n_days
        and scenario_results[1].n_days
    ):
        payload["diff"] = {
    ...
    pd.DataFrame(csv_rows).to_csv(
        base.with_suffix(".csv"), index=False, encoding="utf-8-sig"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Compare baseline vs industry-neutral modeling metrics"
    )
    parser.add_argument(
        "--data-file",
        type=str,
    ...
    for scenario in payload["scenarios"]:
        ...
        print(
            f"  -> {scenario['scenario']}: IC={ic:.4f}, LS_IR={ls_ir:.3f}, LS_ann={ls_ann:.2%}"
        )
    print(
        f"  Outputs: {base_path.with_suffix('.json')} / {base_path.with_suffix('.csv')}"
    )
