...
def detect_factor_columns(df: pd.DataFrame) -> list[str]:
    ...
    if not cols:
        ...
        fallback = [
            ...
            for c in df.columns
            if c.startswith(
                ("momentum_", "volatility_", "volume_ratio", "turnover_rate", "illiq")
            )
        ]
...
def walk_forward_predict(
    ...
    for t in dates:
        ...
        train_df = df.loc[
            train_mask, ["date", "stock_code", "forward_return_1d"] + factor_cols
        ].dropna()
        test_df = df.loc[
            test_mask, ["date", "stock_code", "forward_return_1d"] + factor_cols
...
def walk_forward_predict_regime_specific(
    df: pd.DataFrame,
    regime_factors: dict,
    start_oos: str = "2022-01-01",
    train_window_days: int = 756,
    ...
    for t in dates:
        ...
        train_start = t - pd.Timedelta(days=train_window_days)
        train_mask = (
            (df["date"] >= train_start) & (df["date"] < t) & (df["regime"] == rname)
        )
        test_mask = df["date"] == t
        train_df = df.loc[
            train_mask, ["date", "stock_code", "forward_return_1d"] + fcols
        ].dropna()
        test_df = df.loc[
            test_mask, ["date", "stock_code", "forward_return_1d"] + fcols
        ].dropna(subset=fcols)
...
def evaluate_predictions(
    pred_df: pd.DataFrame, top_n: int = 30, bottom_n: int = 30, cost_bps: float = 0.0
) -> dict:
    # 日度IC
    ics = pred_df.groupby("date").apply(
    ...
    metrics = {
        ...
        "ls_ir": (ls_ret.mean() / (ls_ret.std() + 1e-12)) * np.sqrt(252)
        if len(ls_ret)
        else np.nan,
    }
    return metrics
...
def main():
    ...
    parser.add_argument(
        "--factors-file", type=str, default=None, help="指定因子列表文件（每行一个因子名）"
    )
    ...
    parser.add_argument(
        "--industry-neutral", action="store_true", help="是否按行业进行中性化（需数据包含industry列）"
    )
    ...
    print("📋 数据概览:")
    print(
        f"  观测: {len(df):,}, 股票: {df['stock_code'].nunique()}, 日期: {df['date'].nunique()}"
    )
    ...
    if args.industry_neutral:
        if "industry" not in df.columns:
            ...
            df[factor_cols] = df.groupby(group_keys)[factor_cols].transform(
                lambda x: x - x.mean()
            )
            if "forward_return_1d" in df.columns:
                df["forward_return_1d"] = df.groupby(group_keys)[
                    "forward_return_1d"
    ...
    if (
        use_regime
        and args.regime_factors_file
        and Path(args.regime_factors_file).exists()
    ):
        import json
    ...
    metrics = evaluate_predictions(
        pred_df, top_n=args.top_n, bottom_n=args.bottom_n, cost_bps=args.cost_bps
    )
    print("\n📊 样本外评估:")
