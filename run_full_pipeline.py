...
def run_factor_selection(
    ...
    neutral_industries: Optional[Set[str]],
    ...
    for reg in regimes:
        ...
        selected, summary = select_in_regime(
            sub, factors, ic_thresh=ic_thresh, corr_thresh=corr_thresh
        )
        mapping[reg] = selected
        summary.to_csv(
            DATA_DIR / f"pipeline_factor_summary_{reg}_{ts}.csv",
            index=False,
            encoding="utf-8-sig",
        )
...
def neutralize_dataframe(
    df, factor_cols: List[str], shrink: float, industries: Optional[Set[str]]
):
...
def run_predictive_model(
    ...
    if preds.empty:
        raise RuntimeError(
            "Predictive model produced no predictions (insufficient data)"
        )
    ...
    if "industry" in df.columns:
        ...
        try:
            preds = preds.merge(
                df[meta_cols].drop_duplicates(), on=["date", "stock_code"], how="left"
            )
            # 行业内分位（按 y_pred 降序，越高越靠前；返回0-1）
            preds["ind_count"] = preds.groupby(["date", "industry"])[
                "stock_code"
            ].transform("count")
            preds["ind_rank"] = preds.groupby(["date", "industry"])["y_pred"].rank(
                ascending=False, method="min"
            )
            preds["ind_rank_pct"] = 1.0 - (preds["ind_rank"] - 1) / preds[
                "ind_count"
            ].clip(lower=1)
        except Exception:
            # 安全降级：忽略行业分位计算
    ...
    metrics = evaluate_predictions(
        preds, top_n=top_n, bottom_n=bottom_n, cost_bps=cost_bps
    )
    ...
    regime_contrib = compute_regime_contributions(
        pred_df, df[["date", "regime"]], top_n, bottom_n, cost_bps
    )
    regime_contrib.to_csv(
        Path(report_file).with_name(Path(report_file).stem + "_regime.csv"),
        index=False,
        encoding="utf-8-sig",
    )
...
def run_execution_layer(
    ...
    if strategy == "baseline":
        ...
        ts, scored, details = ema_hysteresis_combo(
            preds,
            top_n=top_n,
            bottom_n=bottom_n,
            cost_bps=cost_bps,
        ...
        ts, details = swap_cap_limited(
            preds,
            top_n=top_n,
            bottom_n=bottom_n,
            cost_bps=cost_bps,
            swap_cap_ratio=swap_cap,
            return_details=True,
        )
        ic_series = compute_ic_series_with_score(preds, "y_pred")
    ...
    if ts is None or ts.empty:
        raise RuntimeError(
            "Execution layer produced empty timeseries — check predictions and parameters"
        )
    ...
    ts_path = (
        Path("data")
        / f"pipeline_execution_{datetime.now().strftime('%Y%m%d_%H%M%S')}_timeseries.csv"
    )
    ts.to_csv(ts_path, index=False, encoding="utf-8-sig")
    metrics_path = ts_path.with_name(
        ts_path.name.replace("_timeseries.csv", "_metrics.json")
    )
        ...
        json.dump(
            {
                ...
                "param": param_desc,
                "metrics": metrics,
                "execution_profile": {**enrich, **half_life},
            },
            f,
            ensure_ascii=False,
            indent=2,
...
def maybe_run_turnover_grid(run: bool, predictions_path: Path, logger) -> Path | None:
    ...
    latest = sorted(
        Path("data").glob("turnover_strategy_grid_*.csv"),
        key=lambda p: p.stat().st_mtime,
    )
    return latest[-1] if latest else None


def maybe_run_cost_grid(
    run: bool,
    regime_file: Path,
    mapping_file: Path,
    start_oos: str,
    ...
    cmd = [
        "python3",
        "analysis/cost_sensitivity_grid.py",
        "--data-file",
        str(regime_file),
        "--mapping-file",
        str(mapping_file),
    ...
    subprocess.run(cmd, check=True)
    latest = sorted(
        DATA_DIR.glob("cost_sensitivity_grid_*.csv"), key=lambda p: p.stat().st_mtime
    )
    return latest[-1] if latest else None
...
def main():
    parser = argparse.ArgumentParser(
        description="Run the full volatility-regime momentum pipeline"
    )
    parser.add_argument(
        "--standardisation", type=str, default="zscore", choices=["zscore", "rank"]
    )
    # 默认使用全量500只股票的股票池
    ...
    parser.add_argument(
        "--run-turnover-grid",
        action="store_true",
        help="After predictions, run turnover grid on the predictions file",
    )
    ...
    if args.neutral_industries:
        industries = {
            item.strip() for item in args.neutral_industries.split(",") if item.strip()
        }
    ...
    if args.recompute_factors:
        ...
        default_path = DATA_DIR / (
            "simple_factor_data_rank.csv"
            if args.standardisation == "rank"
            else "simple_factor_data.csv"
        )
        if not default_path.exists():
            log(
                "Requested reuse of factor data but default file missing; recomputing instead."
            )
            factor_file = recompute_factors(args.standardisation, args.universe)
        else:
    ...
    mapping, mapping_path = run_factor_selection(
        regime_file, args.ic_thresh, args.corr_thresh, args.neutral_shrink, industries
    )
    ...
    grid2 = maybe_run_turnover_grid(
        args.run_turnover_grid, Path(summary["prediction_file"]), log
    )
    if grid2:
