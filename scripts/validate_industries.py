...
def main():
    ...
    for label, paths in [("regime", latest_reg), ("factor", latest_fac)]:
        ...
        print(
            f"{label}: {p.name}  stocks={nunq}  industry_null={null_ratio:.2%}  未分类={unclassified:.2%}"
        )
    ...
    if up.exists():
        ...
        ind_col = (
            ...
            else ("行业" if "行业" in u.columns else None)
        )
        if ind_col:
            null_ratio = u[ind_col].isna().mean()
            unclassified = (u[ind_col].astype(str) == "未分类").mean()
            print(
                f"universe: rows={len(u)}  industry_null={null_ratio:.2%}  未分类={unclassified:.2%}"
            )
        else:
