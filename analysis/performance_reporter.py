...
def compute_portfolio_timeseries(
    pred_df: pd.DataFrame, top_n: int, bottom_n: int, cost_bps: float
) -> pd.DataFrame:
    rows = []
    ...
    for date, group in pred_df.groupby("date"):
        ...
        overlap_short = len(prev_short & curr_short_codes) / max(
            1, len(curr_short_codes)
        )
...
def compute_summary_metrics(ts: pd.DataFrame, ic_series: pd.Series) -> dict:
    metrics = {
        ...
        "ls_ir": float(
            (ts["ls_net"].mean() / (ts["ls_net"].std(ddof=1) + 1e-12)) * np.sqrt(252)
        ),
...
def compute_regime_contributions(
    ...
    for regime, df_reg in merged.groupby("regime"):
        ...
        ic = (
            df_reg.groupby("date")
            .apply(lambda g: g["y_pred"].corr(g["y_true"], method="spearman"))
            .dropna()
        )
        ...
        rows.append(
            {
                ...
                "ls_ir": (ts["ls_net"].mean() / (ts["ls_net"].std(ddof=1) + 1e-12))
                * np.sqrt(252),
                "weight_days": len(ts),
            }
        )
...
def plot_heatmaps(grid_path: Path, metric: str, output_prefix: Path):
    ...
    summary = (
        grid.groupby(["train_window", "top_n", "cost_bps"])
        .agg({metric: "max"})
        .reset_index()
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate portfolio performance reports and visuals"
    )
    parser.add_argument(
        "--predictions", type=str, required=True, help="Predictions CSV path"
    )
    parser.add_argument(
        "--regime-data",
        type=str,
        required=True,
        help="Regime-labelled factor dataset for regime attribution",
    )
    parser.add_argument(
        "--grid-results", type=str, required=True, help="Cost sensitivity grid CSV path"
    )
    ...
    regime_contrib = compute_regime_contributions(
        preds, regime_df, args.top_n, args.bottom_n, args.cost_bps
    )
    ...
    # Save tables
    ts.to_csv(
        prefix.with_name(prefix.name + "_timeseries.csv"),
        index=False,
        encoding="utf-8-sig",
    )
    regime_contrib.to_csv(
        prefix.with_name(prefix.name + "_regime_contrib.csv"),
        index=False,
        encoding="utf-8-sig",
    )
    ...
    print(
        f"  Regime contributions CSV: {prefix.with_name(prefix.name + '_regime_contrib.csv')}"
    )
    ...
    print(
        f"  Heatmaps: {prefix.with_name(prefix.name + '_heatmap_ls_ann.png')}, {prefix.with_name(prefix.name + '_heatmap_ls_ir.png')}"
    )
