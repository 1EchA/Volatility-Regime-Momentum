    ...
    if path:
        ...
        if latest is None:
            raise FileNotFoundError(
                "No volatility_regime_data_*.csv file found in data/"
            )
...
def _inject_rank_standardisation(
    regime_df: pd.DataFrame, rank_factor_file: str
) -> Tuple[pd.DataFrame, int]:
    rank_path = Path(rank_factor_file)
    if not rank_path.exists():
        raise FileNotFoundError(
            f"Rank standardised factor file missing: {rank_factor_file}"
        )
...
def _select_factors(
    df: pd.DataFrame, label: str, ic_thresh: float = 0.01, corr_thresh: float = 0.8
    ...
    for regime in regimes:
        ...
        selected, summary = select_in_regime(
            sub, factor_cols, ic_thresh=ic_thresh, corr_thresh=corr_thresh
        )
...
def run_standardisation_comparison(
    ...
    z_metrics = _run_model(
        base_df, z_mapping, start_oos, train_window, alpha, top_n, bottom_n, cost_bps
    )
    ...
    if missing_rows:
        print(
            f"⚠️ Warning: {missing_rows} rows lack rank-standardised factors after merge."
        )
    r_mapping, r_summary = _select_factors(rank_df, "rank")
    r_metrics = _run_model(
        rank_df, r_mapping, start_oos, train_window, alpha, top_n, bottom_n, cost_bps
    )
    ...
    payload = {
        ...
        "factor_diff": {
            regime: {
                ...
                "only_zscore": sorted(
                    list(
                        set(z_result.selected_factors.get(regime, []))
                        - set(r_result.selected_factors.get(regime, []))
                    )
                ),
                "only_rank": sorted(
                    list(
            ...
            for regime in sorted(
                set(z_result.selected_factors) | set(r_result.selected_factors)
            )
        },
    }
    ...
    pd.DataFrame(summary_rows).to_csv(
        prefix.with_suffix(".csv"), index=False, encoding="utf-8-sig"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Compare z-score vs rank standardisation results"
    )
    parser.add_argument(
        "--zscore-regime-file",
        type=str,
    ...
    parser.add_argument(
        "--rank-factor-file", type=str, default="data/simple_factor_data_rank.csv"
    )
    ...
    for label in ["zscore", "rank"]:
        ...
        print(
            f"  -> {label}: IC={metrics['ic_mean']:.4f}, LS_IR={metrics['ls_ir']:.3f}, LS_ann={metrics['ls_ann']:.2%}"
        )
