...
def baseline_daily(
    ...
    for date, g in df.groupby("date"):
        ...
        daily_sets.append(
            {"date": date, "long": curr_long.copy(), "short": curr_short.copy()}
        )
...
def low_freq_rebalance(
    ...
    for idx, date in enumerate(dates):
        ...
        long_ret = (
            float(pd.Series(list(curr_long)).map(g_map).dropna().mean())
            if curr_long
            else np.nan
        )
        short_ret = (
            float(pd.Series(list(curr_short)).map(g_map).dropna().mean())
            if curr_short
            else np.nan
        )
...
def hysteresis_bands(
    ...
    for date, g in df.groupby("date"):
        ...
        # 收益与成本
        long_ret = (
            float(g[g["stock_code"].isin(curr_long)]["y_true"].mean())
            if curr_long
            else np.nan
        )
        short_ret = (
            float(g[g["stock_code"].isin(curr_short)]["y_true"].mean())
            if curr_short
            else np.nan
        )
        if np.isnan(long_ret) or np.isnan(short_ret):
...
def ema_smoothed(
    ...
    ema_span: int = 5,
    return_details: bool = False,
) -> Tuple[pd.DataFrame, pd.DataFrame] | Tuple[pd.DataFrame, pd.DataFrame, list]:
...
def swap_cap_limited(
    ...
    for date, g in df.groupby("date"):
        ...
        to_add_sorted = sorted(
            to_add, key=lambda s: score_map.get(s, -np.inf), reverse=True
        )
        ...
        to_add_s_sorted = sorted(
            to_add_s, key=lambda s: score_map.get(s, np.inf)
        )  # 低分优先
        to_drop_s_sorted = sorted(
            to_drop_s, key=lambda s: score_map.get(s, -np.inf), reverse=True
        )
...
def compute_ic_series_with_score(
    pred_df: pd.DataFrame, score_col: str = "y_pred"
) -> pd.Series:
    """按指定分数列计算日度IC（Spearman）。避开 pandas 分组警告。"""
    df = _prepare(pred_df)
    # 仅在需要的两列上分组，避免 FutureWarning
    ic = df.groupby("date")[[score_col, "y_true"]].apply(
        lambda g: g[score_col].corr(g["y_true"], method="spearman")
    )
    return ic.dropna()
