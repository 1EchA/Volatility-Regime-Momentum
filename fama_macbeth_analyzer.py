...
class FamaMacBethAnalyzer:
    ...
    def load_and_prepare_data(self):
        ...
        try:
            ...
            self.data = self.data.dropna(
                subset=["forward_return_1d"] + self.factor_columns
            )

            print(f"   ✅ 数据准备完成")
            print(f"   📈 有效观测值: {len(self.data):,} 条")
            print(
                f"   📅 时间跨度: {self.data['date'].min().date()} 至 {self.data['date'].max().date()}"
            )
    ...
    def run_cross_sectional_regressions(self):
        ...
        print(f"   ✅ 横截面回归完成")
        print(
            f"   📊 成功回归日数: {len(self.cross_section_results)} / {self.data['date'].nunique()}"
        )
        print(
            f"   📈 成功率: {len(self.cross_section_results) / self.data['date'].nunique():.1%}"
        )
        ...
        for factor in self.factor_columns:
            ...
            if beta_col in self.cross_section_results.columns:
                ...
                if len(beta_series) > 0:
                    ...
                    beta_pvalue = 2 * (
                        1 - stats.t.cdf(abs(beta_tstat), len(beta_series) - 1)
                    )
                    ...
                    print(
                        f"   📊 {factor}: β={beta_mean:.4f}, t={beta_tstat:.2f}, p={beta_pvalue:.4f} {time_series_results[factor]['significance']}"
                    )
    ...
    def run_diagnostic_tests(self):
        ...
        for factor in self.factor_columns:
            ...
            if beta_col in self.cross_section_results.columns:
                ...
                factor_stability[factor] = {
                    ...
                    "coeff_var": abs(beta_series.std() / beta_series.mean())
                    if beta_series.mean() != 0
                    else np.inf,
                    "positive_days": (beta_series > 0).sum(),
                    "negative_days": (beta_series < 0).sum(),
                    "sign_consistency": max(
                        (beta_series > 0).sum(), (beta_series < 0).sum()
                    )
                    / len(beta_series),
                }
        ...
        diagnostics["sample_adequacy"] = {
            ...
            "obs_to_var_ratio": n_obs_series.mean()
            / (len(self.factor_columns) + 1),  # 观测值/变量比
        }
        ...
        if len(alpha_series) > 10:
            ...
            try:
                lb_stat, lb_pvalue = acorr_ljungbox(
                    alpha_series, lags=10, return_df=False
                )
                diagnostics["residual_analysis"] = {
                    "ljung_box_stat": float(lb_stat.iloc[-1])
                    if hasattr(lb_stat, "iloc")
                    else float(lb_stat),
                    "ljung_box_pvalue": float(lb_pvalue.iloc[-1])
                    if hasattr(lb_pvalue, "iloc")
                    else float(lb_pvalue),
                    "serial_correlation": "Yes"
                    if (lb_pvalue.iloc[-1] if hasattr(lb_pvalue, "iloc") else lb_pvalue)
                    < 0.05
                    else "No",
                }
            except:
        ...
        print("   ✅ 诊断检验完成")
        print(
            f"   📊 R²分布: 均值={diagnostics['r_squared_distribution']['mean']:.4f}, 低R²天数={diagnostics['r_squared_distribution']['low_r2_days']}"
        )
        print(
            f"   🔍 样本充足性: 平均观测值/日={diagnostics['sample_adequacy']['avg_obs_per_day']:.1f}"
        )
    ...
    def generate_comprehensive_report(self):
        ...
        with open(report_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"数据期间: {self.results['cross_section_summary']['date_range'][0]} 至 {self.results['cross_section_summary']['date_range'][1]}\n"
            )
            f.write(
                f"回归次数: {self.results['cross_section_summary']['total_regressions']}\n"
            )
            ...
            if "alpha" in self.results["time_series_results"]:
                ...
                f.write(
                    f"  标准误: {alpha_result['std'] / np.sqrt(alpha_result['n_obs']):.6f}\n\n"
                )
            ...
            # 因子稳定性
            f.write("因子稳定性:\n")
            for factor, stability in self.results["diagnostics"][
                "factor_stability"
            ].items():
    ...
    def save_results(self):
        ...
        self.cross_section_results.to_csv(
            cross_section_file, index=False, encoding="utf-8-sig"
        )
