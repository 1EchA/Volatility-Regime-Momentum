...
def _find_latest_predictions() -> Path | None:
    data_dir = ROOT / "data"
    cands = sorted(
        data_dir.glob("predictions_*.csv"),
...
def run_grid(
    ...
    combo_deltas: Iterable[int] = None,
) -> pd.DataFrame:
    records = []
    ...
    for strat in strategies:
        if strat == "A":  # 低频再平衡
            for top_n, cost, k in product(top_ns, cost_bps_list, k_vals):
                ts = low_freq_rebalance(
                    pred_df, top_n=top_n, bottom_n=top_n, cost_bps=cost, k=int(k)
                )
                if ts.empty:
                    continue
            ...
            for top_n, cost, span in product(top_ns, cost_bps_list, ema_spans):
                ...
                if ts.empty:
                    continue
                ic_series = compute_ic_series_with_score(
                    df_scored.rename(columns={"y_score": "score"}), score_col="score"
                )
                metrics = compute_summary_metrics(ts, ic_series)
                records.append(
                    {
            ...
            for top_n, cost, ema_span, delta in product(
                top_ns, cost_bps_list, combo_ema_spans, combo_deltas
            ):
                ...
                records.append(
                    {
                        "strategy": "E_combo",
                        "param": f"{int(ema_span)}_{int(delta)}",
...
def main():
    parser = argparse.ArgumentParser(
        description="Turnover-optimised execution strategies grid on fixed predictions"
    )
    parser.add_argument(
        "--predictions",
        type=str,
    ...
    parser.add_argument(
        "--strategies", type=str, default="A,B,C,D,E,BASE", help="逗号分隔：A,B,C,D,E,BASE"
    )
    ...
    parser.add_argument(
        "--combo-ema-spans", type=str, default="3,4,6", help="EMA+滞后带组合策略的EMA参数"
    )
    parser.add_argument(
        "--combo-deltas", type=str, default="12,15,18", help="EMA+滞后带组合策略的delta参数"
    )
    # 精细扫描模式
    parser.add_argument(
        "--fine-tune", action="store_true", help="启用精细扫描模式（滞后带5×4×2=40组）"
    )
    parser.add_argument("--combo-only", action="store_true", help="只运行EMA+滞后带组合策略")
    args = parser.parse_args()

    pred_path = (
        Path(args.predictions) if args.predictions else _find_latest_predictions()
    )
    ...
    if args.fine_tune:
        ...
        print(
            f"✓ 精细扫描模式：滞后带策略 {len(delta_vals)}×{len(top_ns)}×{len(cost_bps_list)} = {len(delta_vals)*len(top_ns)*len(cost_bps_list)}组"
        )
    ...
    if args.combo_only:
        ...
        print(
            f"✓ 组合策略模式：EMA+滞后带 {len(combo_ema_spans)}×{len(combo_deltas)}×{len(top_ns)}×{len(cost_bps_list)} = {len(combo_ema_spans)*len(combo_deltas)*len(top_ns)*len(cost_bps_list)}组"
        )
