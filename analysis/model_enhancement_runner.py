    ...
    if path:
        ...
        if not latest:
            raise FileNotFoundError(
                "No volatility_regime_data_*.csv found in data directory."
            )
...
def _neutralize_dataframe(
    df: pd.DataFrame,
    factor_cols: List[str],
    shrink: float,
    industries: Optional[Set[str]],
) -> pd.DataFrame:
...
def _load_mapping(path: str | None = None) -> dict[str, list[str]]:
    if path:
        ...
        if not latest:
            raise FileNotFoundError(
                "No selected_factors_by_regime_*.json mapping found in data directory."
            )
...
def _union_factor_columns(
    mapping: Dict[str, List[str]], fallback_cols: List[str]
) -> List[str]:
...
def _walk_forward_custom(
    ...
    for t in dates:
        ...
        train_mask = (
            (df["date"] >= train_start) & (df["date"] < t) & (df["regime"] == regime)
        )
        test_mask = df["date"] == t
...
def _add_interactions(
    df: pd.DataFrame, mapping: Dict[str, List[str]], max_pairs_per_regime: int = 3
...
def _evaluate(
    pred_df: pd.DataFrame, label: str, top_n: int, bottom_n: int, cost_bps: float
) -> ScenarioMetrics:
    if pred_df.empty:
    ...
    metrics = evaluate_predictions(
        pred_df, top_n=top_n, bottom_n=bottom_n, cost_bps=cost_bps
    )
...
def run_model_enhancements(
    ...
    neutral_industries: Optional[Set[str]] = None,
) -> tuple[dict, Path]:
    ...
    df = _neutralize_dataframe(
        df,
        _union_factor_columns(mapping, fallback_cols),
        neutral_shrink,
        neutral_industries,
    )
    ...
    baseline_metrics = _evaluate(
        baseline_preds, "ridge_baseline", top_n, bottom_n, cost_bps
    )
    ...
    if ElasticNet is not None:
        ...
        for l1_ratio in elastic_net_l1:
            ...
            preds = _walk_forward_custom(
                df, mapping, factory, start_oos, train_window, standardize=True
            )
        ...
        payload_details["elastic_net"] = {
            "l1_grid": list(elastic_net_l1),
            "ls_ir": elastic_metrics,
        }
    else:
        payload_details["elastic_net"] = {"error": "sklearn not available"}

    # Interaction features with ridge
    df_inter, mapping_inter, interaction_cols = _add_interactions(
        df, mapping, max_pairs_per_regime=interaction_pairs
    )
    if interaction_cols and Ridge is not None:
        ...
        metrics_inter = _evaluate(
            preds_inter, "ridge_with_interactions", top_n, bottom_n, cost_bps
        )
        ...
        payload_details["ridge_with_interactions"] = {
            "error": "sklearn Ridge unavailable"
        }
    ...
    if GradientBoostingRegressor is not None:
        ...
        preds_gb = _walk_forward_custom(
            df, mapping, gb_factory, start_oos, train_window, standardize=False
        )
    ...
    payload = {
        ...
        "parameters": {
            ...
            "neutral_industries": sorted(neutral_industries)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Benchmark Elastic Net and interaction feature enhancements"
    )
    ...
    for scenario in payload["scenarios"]:
        print(
            f"  -> {scenario['label']}: IC={scenario['ic_mean']:.4f}, LS_IR={scenario['ls_ir']:.3f}, LS_ann={scenario['ls_ann']:.2%}"
        )
