...
class VolatilityRegimeAnalyzer:
    ...
    def calculate_market_volatility(self, window=None):
        ...
        # 时间序列滚动波动率（等权收益）
        self.market_data["vol_ts"] = self.market_data["market_return"].rolling(
        ...
        if self.vol_metric == "ts":
            ...
            z_ts = (self.market_data["vol_ts"] - self.market_data["vol_ts"].mean()) / (
                self.market_data["vol_ts"].std() + 1e-9
            )
            z_xs = (
                self.market_data["vol_xsec"] - self.market_data["vol_xsec"].mean()
            ) / (self.market_data["vol_xsec"].std() + 1e-9)
    ...
    def merge_regime_data(self):
        ...
        # 选择需要的列
        regime_info = self.market_data[
            ["date", "regime", "volatility", "market_return"]
        ].copy()

        # 合并到主数据
        self.regime_data = pd.merge(self.data, regime_info, on="date", how="inner")
    ...
    def calculate_conditional_momentum(self):
        ...
        for stock_code in self.regime_data["stock_code"].unique():
            stock_data = self.regime_data[
                self.regime_data["stock_code"] == stock_code
            ].copy()
        ...
        self.regime_data = self.regime_data.dropna(
            subset=["forward_return_1d", "momentum_21d"]
        )
        ...
        for regime in ["正常", "高波动", "极高波动"]:
            ...
            if len(regime_subset) > 0:
                stats_dict = {
                    ...
                    "n_stocks_avg": regime_subset.groupby("date")["stock_code"]
                    .count()
                    .mean(),
                }
    ...
    def test_regime_differences(self):
        ...
        if "momentum_anova" in test_results:
            ...
            print(
                f"   - 动量因子制度差异: F={mom_result['f_statistic']:.4f}, p={mom_result['p_value']:.4f} ({sig_text})"
            )
        ...
        if "return_anova" in test_results:
            ...
            print(
                f"   - 前瞻收益率制度差异: F={ret_result['f_statistic']:.4f}, p={ret_result['p_value']:.4f} ({sig_text})"
            )
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="composite",
        choices=["ts", "xsec", "composite"],
    )
    parser.add_argument(
        "--scheme", type=str, default="q50_90", choices=["q33_67", "q50_90"]
    )
